...
def activate_trial(client_id, days=7):
    """Set trial_ends_at and return dashboard token."""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("""
            UPDATE clients
            SET trial_ends_at = NOW() + INTERVAL '%s days',
                plan = 'trial',
                updated_at = NOW()
            WHERE id = %s
            RETURNING dashboard_token, business_name, owner_name, owner_phone, twilio_number
        """ % (days,), (client_id,))
        r = c.fetchone()
        conn.commit()
        if not r:
        ...
        conn.rollback()
        print(f"activate_trial error: {e}")
        return None
    finally:
        conn.close()

def get_trials_ending_soon(days=2):
    """Get clients whose trial ends within X days."""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("""
            SELECT id, business_name, owner_name, owner_phone, twilio_number,
                   dashboard_token, trial_ends_at
            FROM clients
            WHERE plan = 'trial'
            AND active = TRUE
            AND trial_ends_at BETWEEN NOW() AND NOW() + INTERVAL '%s days'
        """ % days)
        rows = c.fetchall()
        return [{
            "id": r[0], "business_name": r[1], "owner_name": r[2],
            "owner_phone": r[3], "twilio_number": r[4],
            "dashboard_token": r[5], "trial_ends_at": str(r[6])
        } for r in rows]
    except Exception as e:
        print(f"get_trials_ending_soon error: {e}")
        return []
    finally:
        conn.close()

def get_trial_day5_clients():
    """Get clients on day 5 of trial — send reminder."""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("""
        ...
        print(f"get_trial_day5_clients error: {e}")
        return []
    finally:
        conn.close()
